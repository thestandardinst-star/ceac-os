...
export default function ManagerFinance({me,openProject}){
 ...
 async function load(){
  ...
  const [b,s,t,p]=await Promise.all([
   ...
   supabase.from("projects").select("id,name,project_units!inner(unit_id)").eq("project_units.unit_id",me.unit_id)
  ]);
 ...
 const unitSpend=useMemo(()=>totals(spend.filter(x=>!x.project_id)),[spend]);
 ...
 return <div className="body">
  ...
  {projects.map(p=>{const pb=totals(budgets.filter(x=>x.project_id===p.id));const ps=totals(spend.filter(x=>x.project_id===p.id));const keys=[...new Set([...Object.keys(pb),...Object.keys(ps)])].sort();return <button key={p.id} className="row" onClick={()=>openProject(p.id)} style={{width:"100%",textAlign:"left"}}>
