...
export function Icon({ name, size = 18, strokeWidth = 1.8, className = "" }) {
  ...
  if (name === "projects" || name === "admin-projects") return <svg {...props}><path d="M3 7h7l2 2h9v10a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z" /><path d="M3 7V5a2 2 0 0 1 2-2h5l2 2h5a2 2 0 0 1 2 2v2" /></svg>;
...
function tabItems(isManager = false) {
  if (isManager) return [["home","Home"],["work","My work"],["team","Team"],["projects","Projects"],["calendar","Calendar"],["strategy","Strategy"],["manager-finance","Finance"],["manager-reports","Reports"],["me","Me"]];
...
function desktopGroups({ isAdmin, isExec, isManager, canPeople = false, canProtectedHR = false, canAudit = false, canAuthority = false, canWorkflows = false, canIntegrations = false }) {
  if (isExec) return [
    { label:"Ministry", items:[["home","Home"],["strategy","Strategy"]] },
    { label:"Communication", items:[["announcements","Announcements"]] },
    { label:"Personal", items:[["me","Me"]] },
  ];
  if (isAdmin) return [
    { label:"Organisation", items:[["home","Home"],["strategy","Strategy"],["units","Units"],["admin-projects","Projects"],["admin-calendar","Calendar"]] },
  ...
  if (isManager) return [
    { label:"Your unit", items:[["home","Home"],["work","My work"],["team","Team"],["strategy","Strategy"],["projects","Projects"],["calendar","Calendar"]] },
...
export function AppTopBar({ me, roleLabel, tab, onProfile }) {
  const titleMap = {
    ...
    finance:"Finance", cost:"Cost", strategy:"Strategy", announcements:"Announcements", audit:"Audit", events:"System events", workflows:"Workflows", authority:"Authority", policies:"Policies & rules", integrations:"Integrations", settings:"Settings", "admin-projects":"Projects", "admin-calendar":"Calendar",
...
export function Tabs({ tab, setTab, isManager, isExec = false, isAdmin = false, me = null }) {
  ...
  const managerGroups = [
    { label:"Planning", items:[["strategy","Strategy"]] },
  ...
  const capabilities = me?.capabilities || [];
  const adminGroups = [
    { label:"Organisation", items:[["strategy","Strategy"],["units","Units"],["admin-projects","Projects"],["admin-calendar","Calendar"]] },
