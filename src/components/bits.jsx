...
export function Icon({ name, size = 18, strokeWidth = 1.8, className = "" }) {
  ...
  if (name === "audit" || name === "authority") return <svg {...props}><path d="M5 4h14v16H5z" /><path d="M8 8h8M8 12h8M8 16h5" /></svg>;
...
function desktopGroups({ isAdmin, isExec, isManager, canPeople = false, canAudit = false, canAuthority = false }) {
  ...
  if (isAdmin) return [
    ...
    { label:"Communication", items:[["announcements","Announcements"]] },
    { label:"System", items:[...(canAudit ? [["audit","Audit"]] : []),...(canAuthority ? [["authority","Authority"]] : []),["settings","Settings"],["me","Me"]] },
  ];
  if (isManager) return [
...
export function AppTopBar({ me, roleLabel, tab, onProfile }) {
  const titleMap = {
    ...
    finance:"Finance", cost:"Cost", announcements:"Announcements", audit:"Audit", authority:"Authority", settings:"Settings", "admin-projects":"Projects", "admin-calendar":"Calendar",
...
export function Tabs({ tab, setTab, isManager, isExec = false, isAdmin = false, me = null }) {
  ...
  const adminGroups = [
    ...
    { label:"Communication", items:[["announcements","Announcements"]] },
    { label:"System", items:[
      ...(capabilities.includes("audit.view") ? [["audit","Audit"]] : []),
