...
export function Icon({ name, size = 18, strokeWidth = 1.8, className = "" }) {
  ...
  if (name === "learning") return <svg {...props}><path d="M4 5.5A3.5 3.5 0 0 1 7.5 2H12v18H7.5A3.5 3.5 0 0 0 4 23z" /><path d="M20 5.5A3.5 3.5 0 0 0 16.5 2H12v18h4.5A3.5 3.5 0 0 1 20 23z" /></svg>;
...
function desktopGroups({ isAdmin, isExec, isManager, canPeople = false, canProtectedHR = false, canAudit = false, canAuthority = false, canWorkflows = false, canIntegrations = false, canWorkload = false, canPerformance = false, canLearning = false }) {
  if (isExec) return [
  ...
  if (isAdmin) return [
    ...
    { label:"People", items:[...(canPeople ? [["people","People"],["lifecycle","Employee lifecycle"]] : []),...(canPerformance ? [["performance","Performance & development"]] : []),...(canLearning ? [["learning","Learning"]] : []),...(canProtectedHR ? [["protected-hr","Protected HR"]] : []),["attendance","Attendance"]] },
  ...
  if (isManager) return [
    { label:"Your unit", items:[["home","Home"],["work","My work"],["team","Team"],["strategy","Strategy"],["delivery","Delivery"],["workload","Workload"],["performance","Performance & development"],["learning","Learning"],["projects","Projects"],["calendar","Calendar"]] },
...
export function Tabs({ tab, setTab, isManager, isExec = false, isAdmin = false, me = null }) {
  ...
  const managerGroups = [
    { label:"Planning", items:[["strategy","Strategy"],["delivery","Delivery"],["workload","Workload"],["performance","Performance & development"],["learning","Learning"]] },
