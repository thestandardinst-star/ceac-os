...
export function Icon({ name, size = 18, strokeWidth = 1.8, className = "" }) {
  ...
  if (name === "audit" || name === "authority" || name === "events" || name === "workflows") return <svg {...props}><path d="M5 4h14v16H5z" /><path d="M8 8h8M8 12h8M8 16h5" /></svg>;
...
function desktopGroups({ isAdmin, isExec, isManager, canPeople = false, canAudit = false, canAuthority = false, canWorkflows = false }) {
  ...
  if (isAdmin) return [
    ...
    { label:"Communication", items:[["announcements","Announcements"]] },
    { label:"System", items:[...(canAudit ? [["audit","Audit"],["events","Events"]] : []),...(canWorkflows ? [["workflows","Workflows"]] : []),...(canAuthority ? [["authority","Authority"]] : []),["settings","Settings"],["me","Me"]] },
  ];
  if (isManager) return [
...
export function AppTopBar({ me, roleLabel, tab, onProfile }) {
  const titleMap = {
    ...
    finance:"Finance", cost:"Cost", announcements:"Announcements", audit:"Audit", events:"System events", workflows:"Workflows", authority:"Authority", settings:"Settings", "admin-projects":"Projects", "admin-calendar":"Calendar",
