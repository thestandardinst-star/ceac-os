  ...
  return <aside className="side premium-side">
    ...
    {(onCreateWork || onCreateMeeting || onMessages) && <section className="reference-quick-create" aria-label="Quick create">
      ...
      <div className="reference-quick-grid">
        ...
        <button onClick={()=>setTab(isAdmin ? "people" : isManager ? "projects" : "me")}>
          <Icon className="premium-icon" name={isAdmin ? "people" : isManager ? "projects" : "hub"} size={15}/>
          <span>{isAdmin ? "Person" : isManager ? "Project" : "Profile"}</span>
        </button>
      </div>
    </section>}
...
function SearchPalette({ query, nav, onNavigate, close }) {
  const q=query.trim().toLowerCase();
  const items=nav
    .map(item=>({ label:item.label, icon:item.icon, action:()=>onNavigate(item.key) }))
  ...
  return <div className="premium-search-popover" role="listbox">
    ...
    {items.length ? items.map((item)=><button key={item.label} onClick={()=>{ item.action(); close(); }}>
      <Icon className="premium-icon" name={item.icon} size={18}/>
      <span>{item.label}</span>
      <Icon className="premium-icon" name="chevron" size={15}/>
    </button>) : <div className="premium-search-empty">No matching destination.</div>}
  </div>;
}
...
export function AppTopBar({
  ...
  return <header className="desktop-topbar premium-topbar" ref={rootRef}>
    <div className="premium-search-wrap">
      ...
      />
      <kbd>⌘ K</kbd>
      {searchOpen && <SearchPalette query={query} nav={nav} onNavigate={onNavigate} close={()=>setSearchOpen(false)}/>}
    </div>
    ...
    <div className="premium-topbar-actions">
      {(onCreateWork || onCreateMeeting || onMessages || onComposeMessage) && <div className="premium-create-wrap">
        <button className="premium-create" onClick={()=>setCreateOpen(v=>!v)}>
          <Icon className="premium-icon" name="plus" size={18}/><span>Create</span>
        </button>
        {createOpen && <div className="premium-create-menu">
          {onCreateWork && <button onClick={()=>{setCreateOpen(false);onCreateWork();}}>
            <Icon className="premium-icon" name="work" size={18}/><span>New work</span>
          </button>}
          {onCreateMeeting && <button onClick={()=>{setCreateOpen(false);onCreateMeeting();}}>
            <Icon className="premium-icon" name="calendar" size={18}/><span>Meeting</span>
          </button>}
          {(onComposeMessage || onMessages) && <button onClick={()=>{setCreateOpen(false);(onComposeMessage || onMessages)?.();}}>
            <Icon className="premium-icon" name="messages" size={18}/><span>Message room</span>
          </button>}
        </div>}
      </div>}

      {onMessages && <button className="premium-icon-button" aria-label="Messages" onClick={onMessages}>
        <Icon className="premium-icon" name="messages" size={19}/>
      </button>}
...
export function MobileTopBar({ me, onProfile, onMessages }) {
  return <header className="mobile-topbar premium-mobile-topbar">
    <div className="premium-mobile-brand">
      <strong>CEAC OS</strong>
      <span>People · Work · Ministry</span>
    </div>
    <div className="premium-mobile-actions">
      {onMessages && <button aria-label="Messages" onClick={onMessages}>
        <Icon className="premium-icon" name="messages" size={19}/>
      </button>}
      <button aria-label="Open profile" onClick={onProfile}>
        <span className="premium-avatar">{(me?.full_name || "C").trim().slice(0,1).toUpperCase()}</span>
      </button>
    </div>
  </header>;
...
export function Tabs({ tab, setTab, isManager, isExec=false, isAdmin=false, me=null }) {
  ...
  const preferred=isAdmin
    ? ["home","people","attendance","finance"]
    : isExec
      ? ["home","work","strategy","delivery"]
      : isManager
        ? ["home","work","team","projects"]
        : ["home","work","team","me"];
  const primary=preferred.map(key=>nav.find(item=>item.key===key)).filter(Boolean);
  const more=nav
    .filter(item=>!primary.some(p=>p.key===item.key))
  ...
  return <>
    {moreOpen && <>
      <button className="premium-mobile-menu-bg" aria-label="Close menu" onClick={()=>setMoreOpen(false)}/>
      <div className="premium-mobile-menu" role="menu">
        <div className="premium-mobile-menu-head"><strong>More</strong><span>{roleName({isAdmin,isExec,isManager})}</span></div>
        {more.map(item=><button role="menuitem" key={item.key} className={tab===item.key?"on":""} onClick={()=>{setMoreOpen(false);setTab(item.key);}}>
          <Icon className="premium-icon" name={item.icon}/><span>{item.label}</span>
        </button>)}
      </div>
    </>}
    ...
    <nav className="tabs premium-tabs" aria-label="Mobile navigation">
      {primary.map(item=><button
        key={item.key}
        aria-label={item.key === "home" ? "Home" : item.key === "me" ? "Me" : item.label}
        className={tab===item.key?"on":""}
        onClick={()=>setTab(item.key)}
      >
        <Icon className="premium-icon" name={item.icon}/>
        <span aria-hidden={item.key === "home" || item.key === "me" ? "true" : undefined}>{item.label}</span>
      </button>)}
      <button className={moreOpen || more.some(item=>item.key===tab) ? "on" : ""} onClick={()=>setMoreOpen(v=>!v)}>
        <Icon className="premium-icon" name="more"/><span>More</span>
      </button>
    </nav>
  </>;
}
