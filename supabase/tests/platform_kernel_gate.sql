...
begin
  ...
  if n<>105 then
    raise exception 'Platform Kernel gate failure: expected 105 authenticated SECURITY DEFINER functions after reviewed migration 095 ministry RPC, found %.',n;
  end if;
  ...
  if to_regprocedure('public.create_ministry_number(uuid,text,text)') is null then
    raise exception 'Platform Kernel gate failure: reviewed migration 095 ministry RPC is missing.';
  end if;

  if not has_function_privilege('authenticated','public.create_ministry_number(uuid,text,text)','EXECUTE')
     or has_function_privilege('anon','public.create_ministry_number(uuid,text,text)','EXECUTE') then
  ...
  if pg_get_functiondef('public.create_ministry_number(uuid,text,text)'::regprocedure) not ilike '%p_unit_id in (select app_managed_units())%'
    ...
    raise exception 'Platform Kernel gate failure: migration 095 ministry RPC does not match reviewed manager/Admin authority.';
  end if;
  ...
  if exists (
    ...
    raise exception 'Platform Kernel gate failure: recurring_operations direct browser writes were reopened.';
  end if;
